...
window.theBooks = books; // for debugging

function _createBooks() {
    books = storageService.load(KEY);
...
function addGoogleBook(googleBook) {
    const book = {
        ...
        publishedDate: googleBook.volumeInfo.publishedDate.substring(0,3),
        ...
        listPrice: {
            amount: utilService.getRandomInt(5, 200),
            currencyCode: "EUR",
            isOnSale: utilService.getRandomBool(),
        },
    };
    books.push(book)
    _saveBooksToStorage()
    return Promise.resolve(book.title);
}

function getNegsIdsByIdx(idx){
    return {prevBookId: books[idx-1].id, nextBookId: books[idx+1].id}
}

function getNextPrevBook(bookId) {
    return _getBookIdx(bookId).then(bookIdx => {
        const negsIds = getNegsIdsByIdx(bookIdx)
        return Promise.resolve(negsIds)
    })
}
